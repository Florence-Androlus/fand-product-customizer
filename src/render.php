...
if ( ! $fandpc_product ) {
	echo '<p><em>' . esc_html__( 'Bloc de personnalisation – affiché sur la page produit.', 'fand-product-customizer' ) . '</em></p>';
	return;
}
...
<div <?php echo wp_kses_post( get_block_wrapper_attributes() ); ?> class="fandpc-preview-wrapper" style="margin-bottom: 20px; border: 1px solid #ddd; padding: 15px;">
	<h3><?php esc_html_e( 'Personnalisez votre produit', 'fand-product-customizer' ); ?></h3>

	<label for="fandpc_text"><strong><?php esc_html_e( 'Votre personnalisation *', 'fand-product-customizer' ); ?></strong></label><br>
	<input
		type="text"
		id="fandpc_text"
		name="fandpc_text"
		...
		required
		style="width: 100%;"
		placeholder="<?php esc_attr_e( 'Entrez le texte', 'fand-product-customizer' ); ?>"
	>
	...
	<div id="fandpc-preview-text" style="font-size: 32px; margin: 15px 0; text-align: center; border: 2px solid #f0f0f0; padding: 20px; background-color: #fafafa; border-radius: 8px; color: #333;">
		<?php esc_html_e( 'Votre texte ici', 'fand-product-customizer' ); ?>
	</div>

	<label for="fandpc_font" style="display: block; margin-top: 10px;"><?php esc_html_e( 'Choisir la police :', 'fand-product-customizer' ); ?></label>
	<select name="fandpc_font" id="fandpc_font" style="width: 100%;">
		<option value=""><?php esc_html_e( '-- Choisissez une police --', 'fand-product-customizer' ); ?></option>
		<?php
